...
class BattleBotConfig(BaseModel):
    ...
    pre_battle_magics: list[str] = Field(description="下一场战斗开始时，期望可用的魔法，用于应对连续无休息的战斗。如果无法使用这里指定的魔法，就会避免进入下一场战斗")
    pre_battle_items: list[str] = Field(description="下一场战斗开始时，期望可用的物品，用于应对连续无休息的战斗。如果无法使用这里指定的傍，就避免进入下一场战斗")
...
def main(args: argparse.Namespace):
    ...
    print("[battle_bot.main] [Info] 本脚本是 thiliapr/hentaiverse_script 的一部分，是一个自由软件，遵循 GNU AGPL v3 or later 进行分发")
